...
namespace LumiTool.BDSPWwiseCloners
{
    public class BgmFieldBothIntroCloner : BaseCloner
    {
        ...
        public override void ExecuteClone(WwiseData wd, string newEventName, WwiseLoopPointData loopData, WwiseLoopPointData dsLoopData)
        {
            ...
            {
                { (0, 1070756379), loopData.CloneForIntro() }, // First segment (Regular)
                { (1, 276947786), loopData.CloneForLoop() },   // Second segment (Regular)
                { (0, 139029859), dsLoopData.CloneForLoop() }, // First segment (DS)
            ...
            {
                { (0, 0, 464817851, 1), loopData.CloneForIntro() },   // First segment, first track, second playlist item (Regular)
                { (0, 1, 612785700, 0), loopData.CloneForIntro() },   // First segment, second track, first playlist item (Regular)
                { (1, 0, 1051238099, 0), loopData.CloneForLoop() },   // Second segment, first track, first playlist item (Regular)

                { (0, 0, 423543179, 0), dsLoopData.CloneForIntro() }, // First segment, first track, first playlist item (DS)
                { (0, 1, 730502391, 1), dsLoopData.CloneForIntro() }, // First segment, second track, second playlist item (DS)
                { (1, 0, 386023986, 0), dsLoopData.CloneForLoop() },  // Second segment, first track, first playlist item (DS)
            });
